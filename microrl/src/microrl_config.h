...
FreshHealth note: This is now appended to whatever prompt is set in the micro_rl_t instance. Weirdly, this could be changed at runtime, but _PROMPT_LEN was (and is) still hardcoded, so I made both hardcoded and use micro_rl_t.prompt_str as a contextual preprompt.
...
memory consuming, depends from _NUM_HISTORY_BYTES parametr */
#define _USE_HISTORY

/*
History buffer length

For saving memory, each entered cmdline store to history in a single buffer,
...
small and embedded devices. Overhead is 1 char on each saved line*/
#define _NUM_HISTORY_BYTES 256
...
#if _NUM_HISTORY_BYTES > 256
#error "This history implementation (buffer with 1 byte iterator) allow 256 byte buffer size maximum"
#endif
